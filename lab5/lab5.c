...
int find_worst_fit(struct header *free_list_ptr, uint64_t size) {
  int worst_fit_id = -1;
  uint64_t worst_fit = 0;
  // TODO: Implement worst fit
  struct header *current = free_list_ptr;
  while (current->next != NULL) {
    if (current->size >= size && current->size > worst_fit) {
...
// loop over all items in free list startin from head
// calculate ptr addr adjacent to left
// calculate ptr addr adjacent to right
// if recently freed block if adjacent to left
// increase size of recently freed block
// adjust next ptr of recently freed block
