...
1. How is the client sending data to the server? What protocol?
   Answer: TCP/IP, write() system call on a connected socket

2. What data is the client sending to the server?
   Answer: "Hello", "Apple", "Car", "Green", "Dog"

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
   Answer:
   - atomic_bool run: a flag to control when the thread should stop accepting connections
   - struct list_handle *list_handle: pointer to the shared list structure for storing messages
   - pthread_mutex_t *list_lock: pointer to the mutex for protecting the shared list

2. How are received messages stored?
   Answer: linked list of list_node (each node has pointers to the next node and message data)

3. What does `main()` do with the received messages?
   Answer: main() waits until all messages are received, then calls collect_all() which 
   prints each message, and frees the allocated memory

4. How are threads used in this sample code?
   Answer: 
   - one acceptor thread listens for client connections
   - each client thread reads messages from its socket and adds them to the shared list

Explain the use of non-blocking sockets in this lab:
- How are sockets made non-blocking?
  Answer: set_non_blocking() function which calls fcntl() with F_SETFL and the O_NONBLOCK flag

- What sockets are made non-blocking?
  Answer: the server listening socket (sfd) and each client connection socket (cfd)

- Why are these sockets made non-blocking? What purpose does it serve?
  Answer: they prevent threads from blocking on I/O ops and deadlocks 
  - accept() returns immediately if no client is waiting, 
  allowing the acceptor thread to check the run flag
  - read() returns immediately if no data is available, 
  allowing threads to continuously poll their sockets while checking
  if they should stop running
*/
